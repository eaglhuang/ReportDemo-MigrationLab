...
| `決策紀錄樣板ADR.md` ADR-016 | 定義 AI / HUMAN / GATE 邊界、週末規則與人類決策範圍。 | Agent Team 派工不得讓 AI 擔任 DRI、closure reviewer 或 ADR / human gate。 |
| `drills/每日任務卡排程.md` | 定義每日 WHAT/WHEN roster。 | Agent Team 每日只依 roster 選卡，不另開平行任務來源。 |
| `runbooks/RB-06-ai-dispatch-cycle.md` | 定義每日 HOW、派工單格式、AI Work Report、review、EOD 與 workstream 標示格式。 | Agent Team 派工、回報與 review 收口依 RB-06 執行。 |
...
| QA / Security / DevOps Agent | 檢查 RBAC、Data Scope、下載閘道、break-glass、network/tool policy 與越權風險。 | 不得降低既有資安標準或把例外改成常態。 |
| QA / Security / DevOps Agent | 確認稽核事件、hash、log、evidence、closure report、fail-closed 條件。 | 不得接受沒有證據的完成聲明。 |
| QA / Security / DevOps Agent | 負責 Golden Dataset、Shadow Validation、新舊系統比對、回歸測試與差異報告。 | 不得使用未核准正式資料做一般開發或測試。 |
...
| QA / Security / DevOps Agent | RBAC、Data Scope、session / token 政策。 | 權限檢查報告、越權測試案例、需 ADR 項目。 | 未授權、跨部門、過期 token、重放攻擊均被拒絕。 | 權限矩陣測試與負向測試。 | Security Lead / Reviewer Agent |
| QA / Security / DevOps Agent | 稽核事件規格、download_id、hash 欄位。 | 下載稽核事件、log 分級、evidence 檢核。 | 每次下載具下載人、時間、IP、User-Agent、報表版本、hash 或查核碼。 | 稽核查詢、hash 檢核、log routing 檢查。 | Audit Lead / Reviewer Agent |
| QA / Security / DevOps Agent | Golden Dataset、測試帳號、舊系統下載案例。 | 新舊下載行為比對與回歸報告。 | 合法下載成功，非法下載被拒，錯誤訊息不洩漏敏感資訊。 | 自動化測試與人工抽核。 | QA Lead |

## 5. M5-02 動態浮水印派工範例

本節為非規範性派工範例，用來示範高風險 PDF 任務應如何納入 QA / Security / DevOps、QA / Security / DevOps 與 QA / Security / DevOps。實際開工規格以 `tasks/README.md`、對應 `TASK-RPT-*` 任務卡與 `drills/分階段演練與驗收計畫.md` 為準。
...
| QA / Security / DevOps Agent | 機密等級分類、PDF permission / encryption 政策。 | 浮水印繞過風險檢查與高機密控制建議。 | 文件不得宣稱 100% 防竄改；需以追蹤、嚇阻、竄改偵測與舉證為定位。 | 風險審查與高機密下載限制測試。 | Security Lead |
| QA / Security / DevOps Agent | Hash、timestamp、簽章、下載稽核欄位。 | 竄改偵測證據、查核碼反查流程與驗證報告。 | 外流文件可反查下載紀錄；簽章或 hash 異常可被偵測。 | hash 驗證、查核碼反查、稽核報表抽測。 | Audit Lead |
| QA / Security / DevOps Agent | Golden PDF、舊系統範例、跨平台 PDF 產出基準。 | Windows / Linux PDF 產出比對、效能與回歸報告。 | 大量產製不造成不可接受延遲，產物差異可解釋。 | 批次產生測試、視覺抽驗、效能測試。 | QA Lead |
...
3. QA / Security / DevOps Agent 與 QA / Security / DevOps Agent 先做初步風險分級。
