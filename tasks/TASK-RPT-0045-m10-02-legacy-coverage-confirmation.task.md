...
確認 Qutora 作為本演練舊系統的必要功能已被新系統覆蓋或有明確例外，包含上傳、metadata、下載、audit、權限與資料搬移。
...
| --- | --- | --- |
| V-0045-01 | 驗證 舊系統必要功能覆蓋矩陣 規格包含 Qutora 輸入、MariaDB 輸出與 evidence 路徑。 | `evidence/ProductionCandidate/TASK-RPT-0045/V-0045-01.md` |
...
| V-0045-10 | 驗證 `git diff --check` 無格式錯誤。 | `evidence/ProductionCandidate/TASK-RPT-0045/V-0045-10.md` |

## Test Cases

| ID | Input | 執行方式 | 預期結果 | 阻擋上線條件 |
| --- | --- | --- | --- | --- |
| TC-0045-01 | 正常 Qutora 合成資料 | 執行舊系統必要功能覆蓋矩陣 happy path | 產出成功結果與 audit event | 無成功 evidence |
...
| TC-0045-09 | 重跑同一批資料 | 重複執行 validator | 結果可重現且 evidence 可追蹤 | 不可重跑或結果漂移 |
| TC-0045-10 | rollback / 回復情境 | 依 RB-04 或任務 rollback 執行 | 可回到 Qutora 或安全狀態 | 回復路徑不可用 |
