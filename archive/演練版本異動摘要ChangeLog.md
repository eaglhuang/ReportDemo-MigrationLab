...
本文件已封存並停止維護。演練異動摘要已併入根目錄 `版本異動摘要ChangeLog.md`；請勿再以本文件作為 active ChangeLog。
...
| 新增 MVP2 兩週調整驗證計畫，列出 MariaDB、下載閘道、動態浮水印與 validator PoC。 | `drills/MVP2兩週調整驗證計畫.md` |
| 新增 Pilot 與平行作業驗證計畫，列出批次移轉、權限、稽核、告警與 rollback dry run。 | `drills/Pilot與平行作業驗證計畫.md` |
| 新增上線候選與 Rollback 計畫，列出 Go / No-Go、backup、restore、break-glass 與正式切換驗證。 | `drills/上線候選與Rollback計畫.md` |
| 新增演練文件索引與引用關係，固定三份主文件、ADR、Agent Team、drills 與 tasks 的引用方向。 | `drills/演練文件索引與引用關係.md` |
| 新增 ADR-012「本演練舊系統採用 Qutora」。 | `決策紀錄樣板ADR.md` |
| 新增 ADR-013「本演練目標資料庫採用 MariaDB」。 | `決策紀錄樣板ADR.md` |
| 新增 ADR-014「兩週 MVP 節奏與完整任務卡開工 Gate」。 | `決策紀錄樣板ADR.md` |
